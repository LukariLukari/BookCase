...
async def connect_client():
    try:
        ...
        print(f"[Telegram Client Connect Error] {type(e).__name__}: {e}")
        return False
...
async def get_telegram_status():
    try:
        ...
        if not ok:
            return {"connected": False, "authorized": False, "username": None, "error": "Cannot connect to Telegram"}
        auth = await client.is_user_authorized()
        username = None
        if auth:
            me = await client.get_me()
            username = getattr(me, 'username', None) or getattr(me, 'first_name', 'Authorized')
        return {"connected": True, "authorized": auth, "username": username, "error": None}
    except Exception as e:
        return {"connected": False, "authorized": False, "username": None, "error": str(e)}
