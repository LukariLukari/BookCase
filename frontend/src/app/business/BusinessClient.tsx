...
import { ArrowLeft, BarChart3, Boxes, CalendarDays, Camera, Check, ChevronDown, CircleDollarSign, ClipboardList, FileSpreadsheet, HelpCircle, Loader2, Minus, PackageOpen, Pencil, Plus, ReceiptText, Search, ShoppingBag, Store, Trash2, TrendingUp, X } from 'lucide-react';
...
type Order = { id:string; code:string; ledger_id:string; customer_name:string; customer_contact?:string; social_link?:string; shipping_fee:number; shipping_cost:number; discount:number; other_fee:number; payment_status:string; note?:string; ordered_at:string; total:number; profit:number; capital_cost:number; item_count:number; items:{product_id:string;product_name:string;quantity:number;unit_price:number;unit_cost:number}[] };
...
const businessMemoryCache=new Map<string,CacheEntry<unknown>>(),CACHE_MAX_AGE=5*60*1000;
function readBusinessCache<T>(key:string):T|null{const memory=businessMemoryCache.get(key) as CacheEntry<T>|undefined;if(memory&&Date.now()-memory.savedAt<CACHE_MAX_AGE)return memory.value;if(typeof window==='undefined')return null;try{const raw=sessionStorage.getItem(`bookcase:${key}`);if(!raw)return null;const entry=JSON.parse(raw) as CacheEntry<T>;if(Date.now()-entry.savedAt>=CACHE_MAX_AGE)return null;businessMemoryCache.set(key,entry);return entry.value}catch{return null}}
function writeBusinessCache<T>(key:string,value:T){const entry={value,savedAt:Date.now()};businessMemoryCache.set(key,entry);if(typeof window!=='undefined')try{sessionStorage.setItem(`bookcase:${key}`,JSON.stringify(entry))}catch{/* Cache is optional when Safari storage is full. */}}
...
function DateField({label,value,set}:{label:string;value:string;set:(v:string)=>void}){const [open,setOpen]=useState(false);const dateObj=value?new Date(value):new Date();const [viewDate,setViewDate]=useState(new Date(dateObj.getFullYear(),dateObj.getMonth(),1));const year=viewDate.getFullYear(), month=viewDate.getMonth();const daysInMonth=new Date(year,month+1,0).getDate(), firstDay=new Date(year,month,1).getDay(), startOffset=firstDay===0?6:firstDay-1;const blanks=Array(startOffset).fill(null), dates=Array.from({length:daysInMonth},(_,i)=>i+1);const display=value?new Intl.DateTimeFormat('vi-VN',{day:'numeric',month:'short',year:'numeric'}).format(new Date(value+'T12:00:00')):'Chọn ngày';return <Field label={label}><div className="relative min-w-0"><button type="button" onClick={()=>setOpen(true)} className={`${input} flex items-center justify-between gap-2 text-left`}><span className={`min-w-0 flex-1 truncate ${value?'text-[#292421]':'text-[#9A8E84]'}`}>{display}</span><CalendarDays size={18} className="shrink-0 text-[#71665E]"/></button>{open&&<div className="fixed inset-0 z-[100] flex items-end bg-black/35 p-0 backdrop-blur-[2px] md:items-center md:justify-center md:p-5" onMouseDown={()=>setOpen(false)}><div className="w-full rounded-t-3xl bg-[#FAF7F2] shadow-2xl md:max-w-sm md:rounded-2xl" onMouseDown={e=>e.stopPropagation()}><div className="flex items-center justify-between border-b border-[#E4D9CE] px-5 py-4"><button onClick={()=>setViewDate(new Date(year,month-1,1))} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#DDD1C5] bg-white"><ChevronDown className="rotate-90" size={18}/></button><b className="text-base text-[#203354]">Tháng {month+1}, {year}</b><button onClick={()=>setViewDate(new Date(year,month+1,1))} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#DDD1C5] bg-white"><ChevronDown className="-rotate-90" size={18}/></button></div><div className="px-5 py-5"><div className="grid grid-cols-7 gap-1 text-center text-[10px] font-black uppercase text-[#8A7E75] mb-3">{['T2','T3','T4','T5','T6','T7','CN'].map(d=><div key={d}>{d}</div>)}</div><div className="grid grid-cols-7 gap-1 text-center text-sm font-semibold text-[#292421]">{blanks.map((_,i)=><div key={`b-${i}`}/>)}{dates.map(d=>{const dStr=`${year}-${String(month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;const isSel=value===dStr;return <button key={dStr} onClick={()=>{set(dStr);setOpen(false)}} className={`flex h-10 w-10 items-center justify-center rounded-full mx-auto ${isSel?'bg-[#203354] text-white':'hover:bg-[#EAE4DD] active:bg-[#DED4C8]'}`}>{d}</button>})}</div></div><div className="flex items-center gap-3 border-t border-[#E4D9CE] px-5 py-4"><button onClick={()=>{set(nowDate());setOpen(false)}} className="flex-1 rounded-xl border border-[#DDD1C5] bg-white px-4 py-3 text-sm font-bold text-[#203354]">Hôm nay</button><button onClick={()=>setOpen(false)} className="flex-1 rounded-xl bg-[#203354] px-4 py-3 text-sm font-bold text-white">Xong</button></div></div></div>}</div></Field>}
type SelectOption={value:string;label:string;description?:string;disabled?:boolean};
function CustomSelect({value,onChange,options,placeholder='Chọn một mục'}:{value:string;onChange:(v:string)=>void;options:SelectOption[];placeholder?:string}){const [open,setOpen]=useState(false),selected=options.find(x=>x.value===value);return <div className="min-w-0"><button type="button" onClick={()=>setOpen(true)} className={`${input} flex items-center justify-between gap-2 text-left`}><span className={`min-w-0 flex-1 truncate ${selected?'text-[#292421]':'text-[#9A8E84]'}`}>{selected?.label||placeholder}</span><ChevronDown size={18} className="shrink-0 text-[#71665E]"/></button>{open&&<div className="fixed inset-0 z-[100] flex items-end bg-black/35 p-0 backdrop-blur-[2px] md:items-center md:justify-center md:p-5" onMouseDown={()=>setOpen(false)}><div className="max-h-[72dvh] w-full overflow-hidden rounded-t-3xl bg-[#FAF7F2] shadow-2xl md:max-w-md md:rounded-2xl" onMouseDown={e=>e.stopPropagation()}><div className="flex items-center justify-between border-b border-[#E4D9CE] px-5 py-4"><b>{placeholder}</b><button type="button" onClick={()=>setOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#DDD1C5] bg-white"><X size={18}/></button></div><div className="max-h-[calc(72dvh-74px)] overflow-y-auto overscroll-contain p-2 pb-[calc(1rem+env(safe-area-inset-bottom))]">{options.map(option=><button type="button" key={option.value} disabled={option.disabled} onClick={()=>{onChange(option.value);setOpen(false)}} className={`flex min-h-14 w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left disabled:opacity-35 ${option.value===value?'bg-[#203354] text-white':'hover:bg-white active:bg-white'}`}><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border ${option.value===value?'border-white bg-white text-[#203354]':'border-[#CFC2B5]'}`}>{option.value===value&&<Check size={14}/>}</span><span className="min-w-0 flex-1"><b className="block truncate text-sm">{option.label}</b>{option.description&&<small className={`mt-0.5 block truncate font-semibold ${option.value===value?'text-white/70':'text-[#7B7067]'}`}>{option.description}</small>}</span></button>)}</div></div></div>}</div>}
...
function Title({small,title,text}:{small:string;title:string;text:string}){return <header className={`relative mb-5 ${small==='Đơn hàng'?'pr-24':'pr-11'}`}><HelpMenu/>{small==='Đơn hàng'&&<button type="button" onClick={()=>window.dispatchEvent(new Event('export-orders'))} aria-label="Xuất Excel" title="Xuất Excel" className="absolute right-11 top-0 flex h-9 w-9 items-center justify-center rounded-full border border-[#D8CCC0] bg-[#FAF7F2] text-emerald-700 shadow-sm"><FileSpreadsheet size={18}/></button>}<p className="text-[11px] font-black uppercase tracking-[.16em] text-[#766B63]">{small}</p><h1 className="mt-1 text-2xl font-black tracking-normal md:text-3xl">{title}</h1><p className="mt-1 text-sm font-medium text-[#6F655E]">{text}</p></header>}
function Nav({view}:{view:View}){const left=[['dashboard','/business','Tổng quan',<Store key="1"/>],['inventory','/business/inventory','Kho hàng',<Boxes key="2"/>]],right=[['orders','/business/orders','Đơn hàng',<ReceiptText key="3"/>],['reports','/business/reports','Thống kê',<BarChart3 key="4"/>]],item=([key,href,label,icon]:ReactNode[]) => <Link key={String(key)} href={String(href)} className={`flex h-12 min-w-0 flex-col items-center justify-center gap-0.5 rounded-xl px-1 text-[9px] font-black sm:text-[10px] md:flex-row md:gap-1.5 md:px-4 md:text-sm [&_svg]:h-[18px] [&_svg]:w-[18px] ${view===key?'bg-[#203354] text-white':'text-[#6D625A]'}`}>{icon}<span className="min-w-0 max-w-full truncate">{label}</span></Link>;return <nav className="fixed inset-x-3 z-40 grid grid-cols-5 items-center rounded-2xl border border-[#DED3C7] bg-[#FAF7F2]/95 p-1.5 shadow-xl backdrop-blur md:static md:mb-5 md:flex md:w-fit md:gap-1 md:rounded-xl md:shadow-sm" style={{bottom:'calc(12px + env(safe-area-inset-bottom))'}}>{left.map(item)}<Link href="/business/orders#new-order" onClick={()=>window.dispatchEvent(new Event('quick-order'))} aria-label="Tạo đơn nhanh" title="Tạo đơn nhanh" className="mx-auto flex h-14 w-14 -translate-y-4 items-center justify-center rounded-full border-[5px] border-[#FAF7F2] bg-[#C6554C] text-white shadow-lg transition active:scale-95 md:mx-1 md:h-12 md:w-12 md:translate-y-0 md:border-0"><Plus size={27} strokeWidth={3}/></Link>{right.map(item)}</nav>}
function Picker({ledgers,id,set,add}:{ledgers:Ledger[];id:string;set:(v:string)=>void;add:()=>void}){return <div className="flex min-w-0 gap-2"><div className="min-w-0 flex-1 md:max-w-sm"><CustomSelect value={id} onChange={set} placeholder="Chọn sổ bán hàng" options={ledgers.map(x=>({value:x.id,label:x.name,description:`Sổ tháng ${x.month} · ${x.order_count} đơn`}))}/></div><button onClick={add} title="Tạo sổ" className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-[#D8CCC0] bg-white"><Plus/></button></div>}
...
export default function BusinessClient({view='dashboard'}:{view?:View}){
 ...
 useEffect(()=>{if(!token||!user)return;router.prefetch('/business');router.prefetch('/business/inventory');router.prefetch('/business/orders');router.prefetch('/business/reports');let ok=true;const prefix=`business:${user.id}`,cachedLedgers=readBusinessCache<Ledger[]>(`${prefix}:ledgers`),cachedProducts=readBusinessCache<Product[]>(`${prefix}:products`),cachedReceipts=readBusinessCache<Receipt[]>(`${prefix}:receipts`),hasCache=!!cachedLedgers;if(cachedLedgers){setLedgers(cachedLedgers);const saved=localStorage.getItem('business_ledger_id');setLedger(cachedLedgers.some(item=>item.id===saved)?saved!:(cachedLedgers[0]?.id||''))}if(cachedProducts&&(view==='inventory'||view==='orders'))setProducts(cachedProducts);if(cachedReceipts&&view==='inventory')setReceipts(cachedReceipts);setLoading(!hasCache);(async()=>{try{const needProducts=view==='inventory'||view==='orders';const [ledgerResponse,productResponse,receiptResponse]=await Promise.all([axios.get<Ledger[]>(`${API}/api/business/ledgers`,{headers}),needProducts?axios.get<Product[]>(`${API}/api/business/products`,{headers}):Promise.resolve({data:[] as Product[]}),view==='inventory'?axios.get<Receipt[]>(`${API}/api/business/stock-receipts`,{headers}):Promise.resolve({data:[] as Receipt[]})]);if(!ok)return;setLedgers(ledgerResponse.data);writeBusinessCache(`${prefix}:ledgers`,ledgerResponse.data);if(needProducts){setProducts(productResponse.data);writeBusinessCache(`${prefix}:products`,productResponse.data)}if(view==='inventory'){setReceipts(receiptResponse.data);writeBusinessCache(`${prefix}:receipts`,receiptResponse.data)}setLedger(current=>{if(current&&ledgerResponse.data.some(item=>item.id===current))return current;const saved=localStorage.getItem('business_ledger_id');return ledgerResponse.data.some(item=>item.id===saved)?saved!:(ledgerResponse.data[0]?.id||'')})}catch(e){if(!hasCache)fail(e,'Không tải được dữ liệu.')}finally{if(ok)setLoading(false)}})();return()=>{ok=false}},[token,user,view,headers,router]);
 ...
 useEffect(()=>{if(!user||loading)return;const prefix=`business:${user.id}`;writeBusinessCache(`${prefix}:ledgers`,ledgers);if(view==='inventory'||view==='orders')writeBusinessCache(`${prefix}:products`,products);if(view==='inventory')writeBusinessCache(`${prefix}:receipts`,receipts)},[user,loading,view,ledgers,products,receipts]);
 ...
 if(view==='orders')return shell(<OrdersV2 {...props} products={products} setProducts={setProducts} orders={orders} setOrders={setOrders} loading={loadingBook}/>);
      ...
      <div className="absolute inset-y-0 right-0 flex items-stretch z-0">
        {onEdit && (
          <button
            ...
            className="flex w-[70px] flex-col items-center justify-center gap-1 bg-[#203354] text-white hover:bg-[#18263F] active:bg-[#121d30] transition-colors cursor-pointer"
            title={editTitle}
          >
            <MinimalEdit size={16} />
        ...
        <button
          ...
          className="flex w-[70px] flex-col items-center justify-center gap-1 bg-[#C4554B] text-white hover:bg-[#b0453c] active:bg-[#9a3b33] transition-colors cursor-pointer"
          title={deleteTitle}
        >
