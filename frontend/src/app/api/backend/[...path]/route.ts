...
async function report(userId:string,ledgerId:string){const ledger=await prisma.businessLedger.findFirst({where:{id:ledgerId,userId},include:{orders:{where:{status:{not:'cancelled'}},include:{items:true}},expenses:true}});if(!ledger)return bad('Không tìm thấy sổ.',404);const totals=ledger.orders.map(orderTotals),revenue=totals.reduce((s,x)=>s+x.total,0),netRevenue=ledger.orders.reduce((s,o,i)=>s+totals[i].subtotal-o.discount,0),capital=totals.reduce((s,x)=>s+x.capital_cost,0),shipping=ledger.orders.reduce((s,o)=>s+o.shippingCost,0),other=ledger.orders.reduce((s,o)=>s+o.otherFee,0),operating=ledger.expenses.reduce((s,e)=>s+e.amount,0),grossProfit=netRevenue-capital,netProfit=revenue-capital-shipping-other-operating;const products=await prisma.businessProduct.findMany({where:{userId,isActive:true}}),stockValue=products.reduce((s,p)=>s+p.stockQuantity*p.unitCost,0),profitAfterInventory=netProfit-stockValue;const daily=new Map<string,{orders:number;revenue:number;profit:number}>(),top=new Map<string,{name:string;quantity:number;revenue:number}>();ledger.orders.forEach((o,i)=>{const d=o.orderedAt.toISOString().slice(0,10),v=daily.get(d)||{orders:0,revenue:0,profit:0};v.orders++;v.revenue+=totals[i].total;v.profit+=totals[i].profit;daily.set(d,v);o.items.forEach(x=>{const v=top.get(x.productId)||{name:x.productName,quantity:0,revenue:0};v.quantity+=x.quantity;v.revenue+=x.quantity*x.unitPrice;top.set(x.productId,v)})});return json({revenue,net_revenue:netRevenue,capital_cost:capital,shipping_cost:shipping,other_order_fee:other,operating_expense:operating,gross_profit:grossProfit,net_profit:netProfit,profit_after_inventory:profitAfterInventory,profit:netProfit,order_count:ledger.orders.length,sold_units:ledger.orders.flatMap(o=>o.items).reduce((s,i)=>s+i.quantity,0),average_order_value:ledger.orders.length?Math.round(revenue/ledger.orders.length):0,stock_units:products.reduce((s,p)=>s+p.stockQuantity,0),stock_value:stockValue,daily:[...daily].map(([date,v])=>({date,...v})).sort((a,b)=>a.date.localeCompare(b.date)),top_products:[...top].map(([product_id,v])=>({product_id,...v})).sort((a,b)=>b.quantity-a.quantity).slice(0,10),expenses:ledger.expenses.map(e=>({id:e.id,category:e.category,amount:e.amount,note:e.note,spent_at:e.spentAt}))})}
